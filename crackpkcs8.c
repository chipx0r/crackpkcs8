...
void generate(workerbrute *wthread, int pivot, X509_SIG *p8, unsigned long long *gcount);
void try(workerbrute *wthread, X509_SIG *p8, unsigned long long *gcount);
...
int main(int argc, char** argv) {
	...
	informat = 0; //Default PEM

	while ((c = getopt (argc, argv, "t:d:s:fvbm:M:c:")) != -1)
		switch (c) {
			case 'f':
				informat = 1; // DER
				break;
...
void *work_dict( void *ptr ) {
	...
	if (!in) {
		fprintf (stderr,"PKCS8 file not found: %s\n",wthread->file2crack);
		exit(10);
	}

	// Creating PKCS8 object
	X509_SIG *p8 = NULL;
	if (informat == 0) {
		if (!(p8 = PEM_read_bio_PKCS8(in, NULL, NULL, NULL))) {		
			perror("Unable to create PKCS8 object\n");
			exit(30);
		}
	} else {
		if (!(p8 = d2i_PKCS8_bio(in, NULL))) {
			perror("Unable to create PKCS8 object\n");
			exit(30);
		}
	}
	...
	*(wthread->count) = 0;
	...
	while (!found && fgets(line, sizeof line,wthread->dictfile) != NULL) {
		...
		if ((p != line) && (*--p == '\r')) *p = '\0';
		(*(wthread->count))++;
		if (PKCS8_decrypt(p8, line, strlen(line)) != NULL)
			found = 1;	
	}
	...
	for (wthread->wordlength=wthread->wordlength_min; wthread->wordlength <= maxwordlength; wthread->wordlength++) {
		for (i=wthread->id; i<wthread->baselength; i+=nthreads) {
			...
			if (wthread->wordlength>1)
				generate(wthread, 1, p8, wthread->count);
			else
				try(wthread, p8, wthread->count);
		}
	}
}
...
void generate(workerbrute *wthread, int pivot, X509_SIG *p8, unsigned long long *gcount) {
	...
	for (i=0; i<wthread->baselength; i++) {
		...
		if (pivot < wthread->wordlength-1)
			generate(wthread, pivot+1, p8, gcount);
		else
			try(wthread,p8,gcount);
	}
...
void try(workerbrute *wthread, X509_SIG *p8, unsigned long long *gcount) {
	...
	if (PKCS8_decrypt(p8, wthread->word, strlen(wthread->word)) != NULL) {
		if (!wthread->quiet) sleep(ELAPSEDSECONDS);
		printf("\n**********************************************************\n");
